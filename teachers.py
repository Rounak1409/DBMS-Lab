...
def TakeImages():
    ...
    assure_path_exists("TeachersDetails/")
    assure_path_exists("TrainingImage/")
    assure_path_exists("BranchDetails/")
    serial = 0
    exists = os.path.isfile("TeachersDetails\TeachersDetails.csv")
    if exists:
        with open("TeachersDetails\TeachersDetails.csv", 'r') as csvFile1:
        ...
        with open("TeachersDetails\TeachersDetails.csv", 'a+') as csvFile1:
    ...
    if (((name.isalpha()) or (' ' in name)) and ((address.isalpha()) or (' ' in address)) and ((college.isalpha()) or (' ' in college)) and ((branch.isalpha()) or (' ' in branch))):
        ...
        with open('TeachersDetails\TeachersDetails.csv', 'a+') as csvFile:
            writer = csv.writer(csvFile)
            writer.writerow(row)
        csvFile.close()
        message1.configure(text=res)
...
def TrackImages():
    ...
    exists1 = os.path.isfile("TeachersDetails\TeachersDetails.csv")
    if exists1:
        df = pd.read_csv("TeachersDetails\TeachersDetails.csv")
    else:
...
def teachers():
    ...
    quitWindow = tk.Button(frame1, text="Back to Guide", command=st.destroy  ,fg="black"  ,bg="red"  ,width=35 ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
